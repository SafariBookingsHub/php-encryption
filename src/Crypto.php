...
class Crypto {
	 ...
	 * @return string
	 */
	private static function encryptInternal(
		$plaintext,
	 ...
	 * @throws Ex\EnvironmentIsBrokenException
	 */
	private static function decryptInternal(
		$ciphertext,
		KeyOrPassword $secret,
		$raw_binary
	) {
		...
		if (self::verifyHMAC($hmac, $header.$salt.$iv.$encrypted,
			$keys->getAuthenticationKey())
		)
		{
			$plaintext = self::plainDecrypt($encrypted,
				$keys->getEncryptionKey(), $iv, Core::CIPHER_METHOD);
	...
	public static function legacyDecrypt(
		...
		if (self::verifyHMAC($hmac, $messageCiphertext, $akey))
		{
			...
			if (Core::ourStrlen($messageCiphertext)
