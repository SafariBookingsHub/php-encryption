...
class CtrModeTest extends TestCase {
	...
	{
		$actual_end = \Defuse\Crypto\Core::incrementCounter(\hex2bin($start),
			$inc);
		$this->assertSame(
			$end,
			\bin2hex($actual_end),
	...
	public function testFuzzIncrementCounter()
	{
		...
		for ($offset = 0; $offset < 16; $offset++)
		{
			 ...
			 *      FF FF FF FF FF 00 00 ... 00
								 ^- offset
			 */
			$start = str_repeat("\xFF", $offset)."\xFE".str_repeat("\xFF",
					16 - $offset - 1);
			$expected_end = str_repeat("\xFF", $offset + 1).str_repeat("\x00",
					16 - $offset - 1);
		...
		for ($trial = 0; $trial < 1000; $trial++)
		{
			...
			$expected_end = $prefix.
				...
				chr(($sum >> 8) & 0xff).
				chr(($sum >> 0) & 0xff);
			$actual_end = \Defuse\Crypto\Core::incrementCounter($start,
				$rand_b);
	...
	public function testCompatibilityWithOpenSSL()
	{
		...
		$cipher_lasthalf = mb_substr($ciphertext, 0x150 * 16, 0x150 * 16,
			'8bit');
